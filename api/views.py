...
def home(request):
    ...
    trendingProducts = products[:4]
    trendLen = len(trendingProducts)
    return render(request, 'home.html', {"products": products, "trendingProducts": trendingProducts, "trendLen": trendLen})
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            product = {"title": title, "description": description, "mainImg": mainImg,
                          "carousel": carousel, "dimensions": dimensions, "category": category, 
